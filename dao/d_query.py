...
from utils.database import get_field, get_table
...
class FilterData(BaseModel):
    ...
    value: Union[int, str, datetime, List[Union[int, str, datetime]]] = Field(..., title='值')
...
def join_query(query: Query, main_table, joins: List[JoinData]) -> Query:
    for join in joins:
        ...
        left_field = get_field(join.on_left, main_table)
        right_field = get_field(join.on_right, right_table)
        if join.method == 'left':
...
def get(query_data: QueryData) -> dict:
...
def filter_items(query_data: FilterQueryData) -> dict:
    tables = [query_data.table]
    if query_data.joins:
        tables.extend([join.table for join in query_data.joins])
    tables = [get_table(table) for table in tables]
...
def group_filter(query_data: FilterGroupQueryData) -> dict:
    main_table = get_table(query_data.table)

    tables = {}
    fields = {}
    stats = {}
    if len(query_data.selects) == 0:
        raise Exception("field selects can not be null")
